...
class TestDeprecateArgument:
    def test_deprecate_argument(self):
        ...
        # Warns and removes
        with pytest.warns(VisibleDeprecationWarning) as record3:
            assert my_foo.bar_arg_none(a=3) == {}
        assert str(record3[0].message) == (
            r"Argument `a` is deprecated and has been removed.  "
            r"To avoid this warning, please do not use `a`. See the "
            r"documentation of `bar_arg_none()` for more details. "
            r"This will not effect behavior."
        )
