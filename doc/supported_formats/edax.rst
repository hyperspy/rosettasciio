...
RosettaSciIO can read ``.spd`` (spectrum image), ``.lsd`` (line scan), and
``.spc`` (single spectra) files from the EDAX TEAM software and its predecessor
EDAX Genesis.
...
If reading an ``.lsd`` file, the calibration of the
line scan is loaded from the corresponding ``.csv`` and ``.spc`` files
stored in the same directory, or from specific files indicated by the user.
If these calibration files are not available, the data from the ``.lsd``
file will still be loaded, but with no spatial or energy calibration.
...
``.spc`` and ``.ipr`` files (if available). If an ``.lsd`` file is loaded, the
result will be a HyperSpy ``EDSSEMSpectrum``, and the calibration will be
loaded from the appropriate ``.spc`` and ``.csv`` files (if available).If an
``.spc`` file is loaded, the result will be a single ``EDSSEMSpectrum`` with no
other files needed for calibration.
